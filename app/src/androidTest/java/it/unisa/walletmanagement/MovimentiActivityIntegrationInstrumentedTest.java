...
@LargeTest
public class MovimentiActivityIntegrationInstrumentedTest {
    @Rule
    public final ActivityTestRule<MovimentiActivity> mActivityRule =
            new ActivityTestRule<>(MovimentiActivity.class, true, false);
    ContoDAO contoDAO;
    ...
    @Test
    public void creaMovimento_CheckSavingInDatabase_True() throws InterruptedException {
        mActivityRule.launchActivity(null);

        // inserimento input
        onView(withId(R.id.button_aggiungi_movimento)).perform(click());
        onView(withId(R.id.spinner_conto)).perform(click());
        onData(allOf(is(instanceOf(String.class)), is("Visa")))
                .inRoot(isPlatformPopup())
                .perform(click());
        onView(withId(R.id.edit_text_nome_movimento)).perform(typeText("Spesa"));
        onView(withId(R.id.edit_text_importo_movimento)).perform(typeText("90"), closeSoftKeyboard());
        onView(withId(R.id.spinner_categoria)).perform(click());
        ...
        onData(anything()).
                inAdapterView(withId(R.id.list_view_movimenti_uscite)).
                atPosition(0).
    ...
    public void aggiornaMovimento_CheckDatabaseUpdate_True() {
        ...
        onView(withId(R.id.button_entrata_movimento)).perform(click());
        onView(withId(R.id.tv_ok)).perform(click());

        // verifica aggiornamento interfaccia
        onData(anything()).
                inAdapterView(withId(R.id.list_view_movimenti_entrate)).
                atPosition(0).
                onChildView(withId(R.id.text_view_nome_movimento)).
        ...
        assertEquals("Il movimento dovrebbe essere stato modificato nel database",
                1, conto.getMovimenti().get(0).getTipo());
    }
